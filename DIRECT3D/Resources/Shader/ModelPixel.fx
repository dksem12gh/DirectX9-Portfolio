...
PS_OUTPUT ps_main(PS_INPUT Input)
{
	...
	float3 diffuseColor = diffTex.rgb * finalDiffuse;
		...
		float3 glossColor = tex2D(Gloss, Input.Texcoord).rgb;
		//
		// Final Color 
		//

		float3 finalColor = diffuseColor  * glossColor + emissionColor;
...
PS_OUTPUT ps_Toon(PS_INPUT Input)
{
	...
	float3 glossColor = tex2D(Gloss, Input.Texcoord).rgb;
	float3 finalColor = diffTex.rgb * finalDiffuse * glossColor;
