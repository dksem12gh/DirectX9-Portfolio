...
struct BONE_MESH : public D3DXMESHCONTAINER
{
	...
	vector<D3DMATERIAL9>			Materials;				//메시의 재질 데이터
	vector<LPDIRECT3DTEXTURE9>		DiffuseTexs;			//로드된 XFile 에서 사용되는 Texture 들
	...
	D3DXMATRIXA16*					pBoneOffsetMatices;		//자신의 기본 행렬 배열
															//D3DXMATRIXA16*				pCurrentBoneMatrices;	//현재 Animation 이 적용된 행렬
...
{
private:
	string			_fileName;		//로딩되는 파일 명
	string			_filePath;		//로딩되는 파일 경로
	string			_texturePath;	// 로딩되는 텍스쳐 경로
	xMeshSkinned*	_pSkinnedMesh;	//자신이 로딩하는 스킨드 메시
